...
Older locks without a `source` field infer the source from their validated URL,
so an explicitly supplied archive directory also works for retained legacy
registry bytes.
