...
for (const name of names) {
  ...
  const archive = path.join(root, 'vendor', `global-torque-${name}-0.1.0.tgz`);
...
for (const ssr of [false, true]) {
  await build({ configFile: false, root, plugins: [vue()], ssr: { noExternal: names.map((name) => `@global-torque/${name}`) }, build: { ssr, outDir: path.join(temporary, ssr ? 'server' : 'client'), emptyOutDir: true, lib: { entry, formats: ['es'], fileName: 'exports' }, rollupOptions: { external: (id) => !id.startsWith('.') && !id.startsWith('/') && !names.some((name) => id.startsWith(`@global-torque/${name}`)) } } });
}
